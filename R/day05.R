...
#' f05b()
f05a <- function(x, cutoff = 8, columns = 9) {
  positions <- seq(2, columns * 4, by = 4)

  stacks <- readr::read_fwf(
    x,
    n_max = cutoff,
  ...
  day_five_a(stacks, directions[[1]])
}


#' @rdname day05
#' @export
f05b <- function(x, cutoff = 8, columns = 9) {
  positions <- seq(2, columns * 4, by = 4)
  ...
  day_five_b(stacks, directions[[1]])
}
